...
class Pieces {
  ...
  async waitFor(piece, timeoutMs, isClosed) {
...
async function waitForFile(path, isClosed, timeoutMs) {
  const deadline = Date.now() + timeoutMs;
  for (;;) {
    try {
...
export async function serveFile(req, res, f, log) {
  const { qbit, hash, path, size, offset, pieceLength, complete } = f;
  let closed = false;
  ...
  const ready =
    (await waitForFile(path, isClosed, FIRST_BYTES_WAIT_MS)) &&
    (await pieces.waitFor(pieceAt(offset, start, pieceLength), FIRST_BYTES_WAIT_MS, isClosed));
  if (!ready) {
    if (!closed) {
  ...
  try {
    ...
    while (pos <= end && !closed) {
      ...
      if (!(await pieces.waitFor(piece, PIECE_WAIT_MS, isClosed))) break;
