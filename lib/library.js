// "Hebits at home": catalog rows and episode lists built from the torrents on disk.
// IDs: hebits:<hebitsId> for the title, hebits:<hebitsId>:<season>:<episode> for episodes.
...
export function parseHebitsId(id) {
  const m = decodeURIComponent(id).match(/^hebits:(\d+)(?::(\d+):(\d+))?$/);
  return m && { hebitsId: m[1], season: m[2] ? +m[2] : undefined, episode: m[3] ? +m[3] : undefined };
}
...
export function catalogMetas(entries, type, { progress, posterUrl }) {
  return entries
    .filter((e) => kindOf(e) === type)
    .sort((a, b) => displayName(a).localeCompare(displayName(b)))
    .map((e) => ({
      id: `hebits:${e.hebitsId}`,
      type,
      name: displayName(e),
      poster: posterUrl(e),
      posterShape: 'poster',
      description: [e.name, statusLine(progress.get(e.hebitsId))].filter(Boolean).join('\n'),
    }));
}

// `extra` is Cinemeta's meta for the IMDb id when known (description, background, ...).
export function metaFor(entry, { progress, posterUrl, extra }) {
  const type = kindOf(entry);
  const meta = {
    id: `hebits:${entry.hebitsId}`,
    type,
    ...
    description: [statusLine(progress), extra?.description, entry.name].filter(Boolean).join('\n\n'),
  ...
  if (type === 'series') {
    ...
    meta.videos = episodes(entry).map((ep, i) => ({
      id: `hebits:${entry.hebitsId}:${ep.season}:${ep.episode}`,
