...
// A supervisor that redirects stdout holds LOG_FILE open in append mode for the life of the
// process, so a rename here would leave that fd writing to the old (now unlinked) inode
// forever. Truncating in place is what makes rotation work with a descriptor this process
// does not own: do NOT "simplify" this into a rename/move, it would silently break rotation.
// It also requires that the supervisor writes to this exact path (cfg.logFile) - otherwise
// rotation trims a file nobody writes while the real log grows without bound.
